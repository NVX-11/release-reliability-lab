...
def render_nginx(backend: str) -> str:
...
def verify_http(url: str, endpoint: str, expected_version: str | None = None) -> None:
    with urlopen(f"{url.rstrip('/')}{endpoint}", timeout=5) as response:
...
def main() -> int:
    ...
    try:
        if args.command == "normalize":
            ...
            args.output.write_text(render_nginx(args.backend))
        else:
            verify_http(args.url, args.endpoint, args.version)
